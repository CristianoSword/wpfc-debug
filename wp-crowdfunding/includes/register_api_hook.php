...
function wpcf_get_submit_form_campaign() {
	...
	//Apoiadores
        $apoiadores = '';
        // Se estiver editando, recuperar o valor existente
        if( isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['postid']) ){
            $post_id = (int) sanitize_text_field($_GET['postid']);
            $apoiadores = get_post_meta( $post_id, 'wpneo_apoiadores', true );
        }

        $html .= '<div class="wpneo-single">';
        $html .= '<div class="wpneo-name">'.__( "Apoiadores" , "wp-crowdfunding" ).'</div>';
        $html .= '<div class="wpneo-fields">';
        $html .= '<input type="number" name="wpneo-form-apoiadores" value="'.$apoiadores.'" min="0" step="1">';
        $html .= '<small>'.__("Número de apoiadores da campanha","wp-crowdfunding").'</small>';
        $html .= '</div>';
        $html .= '</div>';
