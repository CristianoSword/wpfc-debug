...
if ( !class_exists('WPCF_Wallet') ) {
    ...
    class WPCF_Wallet {
        ...
        public function __construct() {
            ...
            add_action ('wp_loaded', array($this, 'wpcf_access_redirect'));
        }

        /**
         * @param $hook
         * @since v.11.1.5
         * redirect if user dont have access
         */
        public function wpcf_access_redirect(){
            if(isset($_GET['payment_campaign_id'])) {
                $author_id = get_post_field( 'post_author', (int)sanitize_text_field($_GET['payment_campaign_id']) );
        ...
        public function wallet_save_setting_callback(){
            if (isset($_POST['wpneo_admin_settings_submit_btn']) && isset($_POST['wpneo_varify_wallet_settings']) && wp_verify_nonce( $_POST['wpneo_settings_page_nonce_field'], 'wpneo_settings_page_action' ) ){
		        ...
		        wpcf_function()->update_checkbox('wpneo_deposit_product_id', $deposit_product_id, ' ');
