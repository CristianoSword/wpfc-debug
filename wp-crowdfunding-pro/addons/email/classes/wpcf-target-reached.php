...
class WPCF_Target_Reached extends WC_Email {
	...
	public function __construct() {
		...
		//add_action( 'wpcf_trigger_campaign_emails', array( $this, 'trigger_emails_on_campaign_date_end' ), 999, 1 );
	...
	public function trigger( $ID, $post ) {
		...
		foreach ( $items as $item ) {
			...
			$donate_total   = $item->get_total();
			$author         = get_userdata( $item->get_product()->post->post_author );
			$dislay_name    = $author->display_name;

			$campaignRunning = wpcf_function()->is_campaign_valid( $campaign_id, $donate_total );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'subject'    => array(
				...
				'placeholder' => __('Campaign [campaign_title] Target is Reached', 'wp-crowdfunding-pro'),
				'default'     => ''
			),
